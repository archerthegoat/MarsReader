...
ensureColumn('tweet_drafts', 'task', "task TEXT NOT NULL DEFAULT 'share'");
...
function normalizeTweetStyle(style) {
  return String(style || '').trim().toLowerCase() === 'reflection'
    ? 'reflection'
    : 'share-rewrite';
}
...
function normalizeTweetTask(task, style = 'share-rewrite') {
  ...
  if (['share', 'polish', 'supplement'].includes(value)) return value;
  return normalizeTweetStyle(style) === 'reflection' ? 'polish' : 'share';
}
...
function saveTweetDraft(entryId, userId, {
  ...
  `).get(id);
  if (!entry) return null;
  const cleanInput = normalizeTweetDraftText(userInput, 2000);
