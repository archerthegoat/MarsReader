...
const DB_FILE = process.env.QMREADER_DB_FILE
  ? path.resolve(process.env.QMREADER_DB_FILE)
  : path.join(DATA_DIR, 'qmreader.sqlite');
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS tweet_drafts (
    ...
    task TEXT NOT NULL DEFAULT 'compose',
    ...
    CHECK(task IN ('compose', 'share', 'polish', 'supplement')),
...
ensureColumn('tweet_drafts', 'task', "task TEXT NOT NULL DEFAULT 'compose'");
ensureColumn('tweet_drafts', 'format', "format TEXT NOT NULL DEFAULT 'short'");
ensureColumn('tweet_drafts', 'tone', "tone TEXT NOT NULL DEFAULT 'natural'");
ensureColumn('tweet_drafts', 'instruction', "instruction TEXT NOT NULL DEFAULT ''");
...
ensureTweetDraftComposeTaskSchema();
...
function ensureTweetDraftComposeTaskSchema() {
  const schema = db.prepare(`
    SELECT sql
    FROM sqlite_master
    WHERE type = 'table' AND name = 'tweet_drafts'
  `).get();
  if (/['"]compose['"]/.test(String(schema && schema.sql || ''))) return;
  db.exec('PRAGMA foreign_keys = OFF');
  db.exec('BEGIN');
  try {
    db.exec(`
      ALTER TABLE tweet_drafts RENAME TO tweet_drafts_old;
      ...
      INSERT INTO tweet_drafts (
        entry_id, user_id, style, task, format, tone, user_input, instruction, draft,
        source_content_hash, model, provider, created_at, updated_at
      )
      SELECT
        entry_id, user_id, style, task, format, tone, user_input, instruction, draft,
        source_content_hash, model, provider, created_at, updated_at
      FROM tweet_drafts_old;
      DROP TABLE tweet_drafts_old;
      CREATE INDEX IF NOT EXISTS idx_tweet_drafts_user_updated
        ON tweet_drafts(user_id, updated_at DESC);
      CREATE INDEX IF NOT EXISTS idx_tweet_drafts_entry
        ON tweet_drafts(entry_id, updated_at DESC);
...
function getContributor(userId, viewer = null) {
  ...
  const contributor = publicContributor(db.prepare(`
    ...
    FROM users
    WHERE id = ? AND COALESCE(disabled_at, 0) = 0
  `).get(id), viewer);
...
function getContributors({ limit = 100, sort = 'latest' } = {}) {
  ...
  return db.prepare(`
    ...
    WHERE COALESCE(u.disabled_at, 0) = 0
      AND (
        COALESCE(t.translation_count, 0) > 0
...
function ensureAdminUser({ email, password, displayName = '向阳乔木' }) {
  ...
  if (row) {
    db.prepare(`
      UPDATE users
      SET display_name = ?, role = 'admin', password_hash = ?, password_salt = ?,
          disabled_at = NULL, disabled_by = NULL, disabled_reason = NULL, updated_at = ?
      WHERE email = ?
    `).run(publicAuthor(displayName), record.hash, record.salt, t, normalizedEmail);
  } else {
    db.prepare(`
      INSERT INTO users (id, email, display_name, default_reader_tab, role, password_hash, password_salt, created_at, updated_at)
      VALUES (?, ?, ?, 'original', 'admin', ?, ?, ?, ?)
...
function getUserNotifications(userId, { limit = 80 } = {}) {
  ...
  return db.prepare(`
    SELECT n.*, a.display_name AS actor_name, a.avatar_url AS actor_avatar,
           ...
           tr.title_hash AS translation_title_hash
  ...
  `).all(id, safeLimit).map(row => {
    const translatedTitle = validTranslatedTitle(row, 'entry_title', 'entry_title_zh');
    const message = !translatedTitle && row.entry_title_zh
      ? String(row.message || '').split(row.entry_title_zh).join(row.entry_title || '文章')
      : row.message;
    return {
      id: row.id,
      type: row.type,
      ...
      entryTitle: translatedTitle || row.entry_title || '',
...
function validTranslatedTitle(row, titleKey = 'title', translatedKey = 'title_zh', hashKey = 'translation_title_hash') {
  const translated = String(row && row[translatedKey] || '').trim();
  if (!translated) return null;
  const savedHash = String(row && row[hashKey] || '').trim();
  if (savedHash !== hashText(row && row[titleKey] || '')) return null;
  return translated;
}
...
function normalizeEntry(row) {
  ...
  return {
    ...
    title: row.title,
    titleZh: validTranslatedTitle(row),
    link: row.link || '',
    author: row.author || '',
...
function getEntry(id) {
  return normalizeEntry(db.prepare(`
    SELECT e.*, t.title_zh, t.summary_zh, t.title_hash AS translation_title_hash
    FROM entries e
...
function getEntriesByIds(entryIds) {
  ...
  for (let offset = 0; offset < ids.length; offset += 400) {
    ...
    const rows = db.prepare(`
      ...
      FROM entries e
      LEFT JOIN entry_translations t ON t.entry_id = e.id
      WHERE e.id IN (${placeholders}) AND COALESCE(e.deleted_at, 0) = 0
    `).all(...chunk);
    out.push(...rows.map(normalizeEntry));
  }
  return out;
}

const ENTRY_RETENTION_PROTECTION_SQL = `
      AND NOT EXISTS (
        SELECT 1 FROM user_entry_states s
        WHERE s.entry_id = e.id AND s.starred_at IS NOT NULL
      )
      AND NOT EXISTS (SELECT 1 FROM entry_translations t WHERE t.entry_id = e.id)
      AND NOT EXISTS (SELECT 1 FROM entry_rewrites r WHERE r.entry_id = e.id)
      AND NOT EXISTS (SELECT 1 FROM entry_ai_asset_contributions a WHERE a.entry_id = e.id)
      AND NOT EXISTS (SELECT 1 FROM commentaries c WHERE c.entry_id = e.id)
      AND NOT EXISTS (SELECT 1 FROM text_annotations a WHERE a.entry_id = e.id)
      AND NOT EXISTS (SELECT 1 FROM chat_messages m WHERE m.entry_id = e.id)
      AND NOT EXISTS (SELECT 1 FROM tweet_drafts d WHERE d.entry_id = e.id)
`;

function getEntryRetentionCandidates({ cutoffTs } = {}) {
  ...
  return db.prepare(`
    ...
    FROM entries e
    WHERE (
      CASE WHEN COALESCE(e.published_ts, 0) > 0 THEN e.published_ts ELSE e.created_at END
    ) < ?
      ${ENTRY_RETENTION_PROTECTION_SQL}
    ORDER BY e.published_ts ASC, e.created_at ASC
  `).all(cutoff).map(row => row.id).filter(Boolean);
}

function deleteEntriesByIds(entryIds, { cutoffTs = null } = {}) {
  const ids = [...new Set((entryIds || []).map(normalizeEntryId).filter(Boolean))];
  ...
  try {
    for (let offset = 0; offset < ids.length; offset += 400) {
      ...
      const placeholders = chunk.map(() => '?').join(',');
      const cutoff = Number(cutoffTs);
      const retentionGuard = Number.isFinite(cutoff) && cutoff > 0
        ? ` AND (CASE WHEN COALESCE(e.published_ts, 0) > 0 THEN e.published_ts ELSE e.created_at END) < ? ${ENTRY_RETENTION_PROTECTION_SQL}`
        : '';
      const values = Number.isFinite(cutoff) && cutoff > 0 ? [...chunk, cutoff] : chunk;
      deletedCount += Number(db.prepare(`DELETE FROM entries AS e WHERE e.id IN (${placeholders})${retentionGuard}`).run(...values).changes) || 0;
    }
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
...
function createSubmissionRequest({ url = '', userId = '', author = '', note = '' } = {}) {
  ...
  const user = cleanUserId ? db.prepare('SELECT id, email, display_name, disabled_at FROM users WHERE id = ?').get(cleanUserId) : null;
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)
  `).run(id, cleanUrl, cleanUserId, publicAuthor(author || user.display_name || user.email), String(note || '').trim().slice(0, 500), t, t);
...
function reviewSubmissionRequest(id, { status = '', reviewedBy = '', reason = '', entryId = '' } = {}) {
  ...
  `).run(status, t, String(reviewedBy || '').trim() || null, String(reason || '').trim().slice(0, 300) || null, String(entryId || '').trim() || null, t, request.id);
...
function getTitleTranslations(ids) {
  ...
  const out = {};
  for (let offset = 0; offset < cleanIds.length; offset += 400) {
    const chunk = cleanIds.slice(offset, offset + 400);
    const placeholders = chunk.map(() => '?').join(',');
    const rows = db.prepare(`
      SELECT t.entry_id, t.title_zh, t.title_hash, e.title
      FROM entry_translations t
      JOIN entries e ON e.id = t.entry_id
      WHERE t.entry_id IN (${placeholders})
        AND t.title_zh IS NOT NULL
        AND t.title_zh <> ''
    `).all(...chunk);
    for (const row of rows) {
      if (row.title_hash !== hashText(row.title || '')) continue;
      out[row.entry_id] = row.title_zh;
    }
  }
  return out;
}
...
function getEntryAssetSummaries(ids) {
  ...
  for (const row of db.prepare(`
    SELECT c.entry_id, c.id, c.asset_type, c.author, c.title, c.summary, c.content_json,
           c.body, c.model, c.title_hash, c.created_at, c.updated_at,
           e.title AS entry_title, u.display_name AS contributor_name,
...
function getTranslation(entryId) {
  const row = db.prepare(`
    SELECT t.*, e.title AS entry_title, u.display_name AS contributor_name, c.id AS asset_id
    FROM entry_translations t
    JOIN entries e ON e.id = t.entry_id
  ...
  return {
    ...
    titleZh: validTranslatedTitle(row, 'entry_title', 'title_zh', 'title_hash') || '',
...
function saveTranslation(entryId, translation) {
  ...
  db.prepare(`
    ...
    ON CONFLICT(entry_id) DO UPDATE SET
      ...
      title_zh = CASE
        WHEN excluded.title_zh <> '' THEN excluded.title_zh
        WHEN entry_translations.title_hash = excluded.title_hash THEN entry_translations.title_zh
        ELSE ''
      END,
...
function getAiAssetContribution(assetId, type = '') {
  ...
  const row = db.prepare(`
    SELECT c.*, e.title AS entry_title, u.display_name AS contributor_name
    FROM entry_ai_asset_contributions c
    JOIN entries e ON e.id = c.entry_id
    LEFT JOIN users u ON u.id = c.user_id
    WHERE c.id = ?
  ...
  if (row.asset_type === 'translation') {
    return {
      ...base,
      titleZh: validTranslatedTitle(row, 'entry_title', 'title', 'title_hash') || '',
...
function aiAssetContributionPreview(row, helpfulCount = 0) {
  ...
  const base = {
    ...
    title: type === 'translation'
      ? validTranslatedTitle(row, 'entry_title', 'title', 'title_hash') || ''
      : row.title || '',
    model: row.model || '',
    text: '',
    at,
...
function getUserComments(userId, { limit = 100 } = {}) {
  ...
  return db.prepare(`
    SELECT c.id, c.entry_id, c.author, c.body, c.model, c.created_at, c.updated_at,
           ...
           t.title_zh, t.title_hash AS translation_title_hash,
           u.display_name AS contributor_name,
           COALESCE(r.helpful_count, 0) AS helpful_count
    FROM commentaries c
    JOIN entries e ON e.id = c.entry_id
...
function getUserEntryReactions(userId, { limit = 100, reaction = 'like' } = {}) {
  ...
  const rows = db.prepare(`
    SELECT r.entry_id, r.reaction, r.created_at, r.updated_at,
           ...
           e.summary, e.image,
           t.title_zh, t.summary_zh, t.title_hash AS translation_title_hash
...
function getUserTranslations(userId, { limit = 100 } = {}) {
  ...
  `).all(id, safeLimit).map(row => {
    ...
    return {
      ...
      contributorName: row.contributor_name || row.author || '',
      author: row.author || row.contributor_name || '',
      titleZh: validTranslatedTitle(row, 'title', 'title_zh', 'title_hash') || '',
      summaryZh: row.summary_zh || '',
...
function getUserRewrites(userId, { limit = 100 } = {}) {
  ...
  return db.prepare(`
    SELECT rw.id AS asset_id, rw.entry_id, rw.title AS rewrite_title, rw.body, rw.model, rw.provider,
           ...
           e.source_id, e.title, e.link, e.published, e.published_ts,
           tr.title_zh, tr.title_hash AS translation_title_hash,
           u.display_name AS contributor_name,
           COALESCE(r.helpful_count, 0) AS helpful_count
...
function getAnnotationNotificationTarget(entryId, annotationId, suffix = '觉得') {
  const row = db.prepare(`
    SELECT a.id, a.user_id, a.quote, a.body, e.title, t.title_zh,
           t.title_hash AS translation_title_hash
    FROM text_annotations a
    JOIN entries e ON e.id = a.entry_id
    LEFT JOIN entry_translations t ON t.entry_id = e.id
  ...
  return row ? {
    userId: row.user_id || '',
    objectId: row.id,
    message: `有人${suffix}你的划线点评：${assetSnippet(row.quote || validTranslatedTitle(row) || row.title || row.body, 80)}`,
  } : null;
}
...
function getCommentNotificationTarget(entryId, commentId) {
  ...
  return row ? {
    userId: row.user_id || '',
    objectId: row.id,
    message: `有人觉得你的点评有用：${assetSnippet(validTranslatedTitle(row) || row.title || row.body, 80)}`,
  } : null;
}
...
function getChatNotificationTarget(entryId, messageId) {
  ...
  return row ? {
    userId: row.user_id || '',
    objectId: row.id,
    message: `有人觉得你的文章对话有用：${assetSnippet(validTranslatedTitle(row) || row.title || row.content, 80)}`,
  } : null;
}
...
function getEntryAssetNotificationTarget(entryId, type, assetId = '') {
  ...
  if (aiAssetId) {
    const row = db.prepare(`
      SELECT c.id, c.user_id, c.title AS asset_title, c.title_hash AS asset_title_hash,
             c.body, c.summary, e.title AS entry_title,
             t.title_zh AS entry_title_zh, t.title_hash AS translation_title_hash
      FROM entry_ai_asset_contributions c
      JOIN entries e ON e.id = c.entry_id
      LEFT JOIN entry_translations t ON t.entry_id = e.id
    ...
    if (!row) return null;
    const entryTitleZh = validTranslatedTitle(row, 'entry_title', 'entry_title_zh');
    const assetTitle = assetType === 'translation'
      ? validTranslatedTitle(row, 'entry_title', 'asset_title', 'asset_title_hash')
      : row.asset_title;
    return {
      userId: row.user_id || '',
      objectId: row.id,
      message: `有人觉得你的${assetType === 'translation' ? '中文翻译' : '中文改写'}有用：${assetSnippet(entryTitleZh || row.entry_title || assetTitle || row.summary || row.body, 80)}`,
    };
  }
  const row = assetType === 'translation'
    ? db.prepare(`
      SELECT tr.user_id, tr.title_zh, tr.title_hash AS translation_title_hash,
    ...
    `).get(id)
    : db.prepare(`
      SELECT rw.user_id, rw.title AS asset_title, rw.body, e.title AS entry_title,
             tr.title_zh, tr.title_hash AS translation_title_hash
  ...
  return row ? {
    userId: row.user_id || '',
    objectId: '',
    message: `有人觉得你的${assetType === 'translation' ? '中文翻译' : '中文改写'}有用：${assetSnippet(validTranslatedTitle(row, 'entry_title') || row.entry_title || row.asset_title || row.body, 80)}`,
  } : null;
}
...
function getEntrySubmissionOwner(entryId) {
  ...
  return row ? {
    ...
    message: `有人反馈了你提交的链接：${assetSnippet(validTranslatedTitle(row) || row.title, 80)}`,
  } : null;
}
...
function normalizeTweetStyle(style) {
  const value = String(style || '').trim().toLowerCase();
  return ['share-rewrite', 'reflection'].includes(value)
    ? value
    : 'share-rewrite';
}

function normalizeTweetTask(task, style = 'share-rewrite') {
  const value = String(task || '').trim().toLowerCase();
  if (['compose', 'share', 'polish', 'supplement'].includes(value)) return value;
  if (normalizeTweetStyle(style) === 'reflection') return 'polish';
  if (normalizeTweetStyle(style) === 'share-rewrite') return 'share';
  return 'compose';
}

function normalizeTweetFormat(format) {
  const value = String(format || '').trim().toLowerCase();
  if (value === 'bullets' || value === 'thread') return 'long';
  return ['short', 'long'].includes(value) ? value : 'short';
}

function normalizeTweetTone(tone) {
  const value = String(tone || '').trim().toLowerCase();
  return ['natural', 'restrained', 'pointed'].includes(value) ? value : 'natural';
}

function normalizeTweetDraftText(value, max = 20000) {
  return String(value || '').replace(/\u0000/g, '').trim().slice(0, max);
}

function mapTweetDraft(row, { entry = null } = {}) {
  if (!row) return null;
  const currentHash = String(entry && entry.contentHash || row.entry_content_hash || '').trim();
  const sourceHash = String(row.source_content_hash || '').trim();
  return {
    entryId: row.entry_id,
    userId: row.user_id || '',
    style: normalizeTweetStyle(row.style),
    task: normalizeTweetTask(row.task, row.style),
    format: normalizeTweetFormat(row.format),
    tone: normalizeTweetTone(row.tone),
    ...
    stale: Boolean(sourceHash && currentHash && sourceHash !== currentHash),
  };
}

function getTweetDraft(entryId, userId) {
  const id = normalizeEntryId(entryId);
  const ownerId = String(userId || '').trim();
  if (!id || !ownerId) return null;
  const row = db.prepare(`
    SELECT d.entry_id, d.user_id, d.style, d.task, d.format, d.tone, d.user_input, d.instruction, d.draft,
           d.source_content_hash, d.model, d.provider, d.created_at, d.updated_at,
           e.content_hash AS entry_content_hash
    FROM tweet_drafts d
    JOIN entries e ON e.id = d.entry_id
    WHERE d.entry_id = ? AND d.user_id = ?
      AND COALESCE(e.deleted_at, 0) = 0
  `).get(id, ownerId);
  return mapTweetDraft(row);
}

function getUserTweetDrafts(userId, { limit = 100 } = {}) {
  const ownerId = String(userId || '').trim();
  if (!ownerId) return [];
  const safeLimit = Math.max(1, Math.min(200, Number.parseInt(limit, 10) || 100));
  return db.prepare(`
    ...
    LIMIT ?
  `).all(ownerId, safeLimit).map(row => ({
    ...mapTweetDraft(row),
    draftSnippet: assetSnippet(row.draft, 220),
    entry: {
      id: row.entry_id,
      sourceId: row.source_id,
      title: row.title,
...
function saveTweetDraft(entryId, userId, {
  style = 'share-rewrite',
  task = 'compose',
  format = 'short',
  tone = 'natural',
  ...
  `).get(id);
  if (!entry) return null;
  const cleanInput = normalizeTweetDraftText(userInput, 3000);
  const cleanInstruction = normalizeTweetDraftText(instruction, 800);
  const cleanDraft = normalizeTweetDraftText(draft, 20000);
  if (!cleanInput && !cleanDraft) {
    deleteTweetDraft(id, ownerId);
    return null;
  }
  const cleanHash = normalizeTweetDraftText(sourceContentHash || entry.content_hash || '', 160);
  const cleanModel = normalizeTweetDraftText(model, 160);
  const cleanProvider = normalizeTweetDraftText(provider || 'deepseek', 80) || 'deepseek';
  const t = now();
  db.prepare(`
    INSERT INTO tweet_drafts (
      entry_id, user_id, style, task, format, tone, user_input, instruction, draft, source_content_hash,
  ...
  `).run(
    id,
    ownerId,
    normalizeTweetStyle(style),
    normalizeTweetTask(task, style),
    normalizeTweetFormat(format),
    normalizeTweetTone(tone),
    cleanInput,
    cleanInstruction,
    cleanDraft,
    cleanHash,
  ...
  return getTweetDraft(id, ownerId);
}

function deleteTweetDraft(entryId, userId) {
  const id = normalizeEntryId(entryId);
  const ownerId = String(userId || '').trim();
  if (!id || !ownerId) return false;
  return Boolean(db.prepare('DELETE FROM tweet_drafts WHERE entry_id = ? AND user_id = ?').run(id, ownerId).changes);
}
