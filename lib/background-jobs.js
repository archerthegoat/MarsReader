  ...
  for (const entry of targets) {
    ...
    if (entry.sourceId === 'producthunt' && !prepared.officialSiteFetched) {
      skippedItems.push({
        entryId: entry.id,
        title: entry.title,
        reason: existing ? '官网抓取失败，保留已有改写并标记为待复核' : '官网抓取失败，暂不生成低质量改写',
      });
      continue;
    }
...
async function runRefreshJob(job = {}, hooks = {}) {
  ...
  if (kind === 'auto-rewrite' || kind === 'title-translation') {
    ...
    const autoRewrite = kind === 'title-translation'
      ? { rewritten: 0, cached: 0, failed: [], changed: 0, skipped: 'manual-only' }
      : await autoRewriteSources(sourceIds);
  ...
  if (sourceId) {
    ...
    refresh = {
      ...
      refreshed: completed,
      status,
      error: nonOkItems.length
        ? `${nonOkItems.length}/${completed.length} 个信息源未成功刷新`
        : null,
      okCount,
      staleCount,
  ...
  if (!AUTO_REWRITE_ENABLED) {
    autoRewrite = { rewritten: 0, cached: 0, failed: [], changed: 0, skipped: 'manual-only' };
  } else if (sourceId) {
