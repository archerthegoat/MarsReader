...
process.env.AUTO_REWRITE_ENABLED = '0';
...
test('refresh jobs do not generate Chinese rewrites when rewrite automation is disabled', async () => {
  let rewriteCalls = 0;
  const restoreFetcher = stub(fetcher, {
    ...
    fetchSource: async source => ({
      status: 'ok',
      entries: [{ id: 'entry-1' }],
      changedEntries: [{ id: 'entry-1', sourceId: source.id }],
    }),
  });
  const restoreDeepseek = stub(deepseek, {
    ...
    rewriteEntry: async () => {
      rewriteCalls += 1;
      throw new Error('rewrite should remain manual');
    },
  });
  try {
    const result = await jobs.runRefreshJob({
    ...
    assert.equal(result.autoRewrite.skipped, 'manual-only');
