/* Mars Reader front-end */
const $ = (s, el = document) => el.querySelector(s);
const $$ = (s, el = document) => [...el.querySelectorAll(s)];
const PRODUCT_NAME = 'Mars Reader';
const PRODUCT_TITLE = `${PRODUCT_NAME} · RSS 阅读器`;
...
const ASSET_FOCUS_LABELS = { translation: '中文改写', rewrite: '中文改写', annotations: '划线点评', comments: '人工点评', chat: '文章对话' };
const TRANSLATION_REWRITE_LABEL = '中文改写（对照）';
const ANNOTATION_SURFACE_LABELS = { original: '原文', rewrite: '中文改写', translation: TRANSLATION_REWRITE_LABEL };
...
function initialTweetSystemPrompt() {
  ...
  if (!stored || looksLikeLegacyDefault) {
    ...
    return DEFAULT_TWEET_SYSTEM_PROMPT;
  }
  if (version !== TWEET_SYSTEM_PROMPT_VERSION) storage.setItem(TWEET_SYSTEM_PROMPT_VERSION_KEY, TWEET_SYSTEM_PROMPT_VERSION);
  return stored;
}
...
const state = {
  ...
  // The writing desk owns the right-side width now; keep the legacy field
  // inert so an old manually-resized value cannot override the responsive grid.
  contextPaneWidth: null,
  me: null,
  authBypass: false,
  authMode: 'login',
  aiProfiles: [],
  ...
  loadedAiScope: '',
  tweetDraft: '',
  tweetDraftBusy: false,
  tweetDraftLoading: false,
  tweetDraftSaving: false,
  tweetDraftRequestSeq: 0,
  tweetDraftAbortController: null,
  tweetDraftLoadController: null,
...
function routeStateFromUrl() {
  ...
  return {
    ...
    tab: routeReaderTab,
    view: contributorsPath
      ? 'contributors'
      : (isAssetPath || params.get('view') === 'assets'
        ? 'assets'
        : params.get('view') === 'tweet-drafts' ? 'tweet-drafts' : ''),
...
function listRouteTitle(view = state.view, assetFilter = state.assetFilter, q = state.q) {
  if (view === 'contributors') {
    ...
    return q ? `${sortPrefix}贡献榜 · “${q}” · ${PRODUCT_NAME}` : `${sortPrefix}贡献榜 · ${PRODUCT_NAME}`;
  }
  if (view === 'assets') {
    const sortPrefix = state.assetSort === 'helpful' ? '有用 · ' : '';
    const prefix = `${sortPrefix}${assetFilter ? `${assetDirectoryLabel(assetFilter)}资产` : '公开资产'}`;
    return q ? `${prefix} · “${q}” · ${PRODUCT_NAME}` : `${prefix} · ${PRODUCT_NAME}`;
  }
  if (view === 'tweet-drafts') {
    return q ? `推文草稿 · “${q}” · ${PRODUCT_NAME}` : `推文草稿 · ${PRODUCT_NAME}`;
  }
...
function readerRouteTitle(entry = state.activeEntry, focus = state.readerFocus) {
  ...
  return `${prefix}${title} · ${PRODUCT_NAME}`;
}
...
function listUrlFor(view = state.view, assetFilter = state.assetFilter) {
  ...
  } else if (view === 'tweet-drafts') {
    url.searchParams.set('view', 'tweet-drafts');
  }
  return url;
}
...
function avatarInitial(user) {
  return ((user && (user.displayName || user.email)) || 'M').trim().slice(0, 1).toUpperCase() || 'M';
}
...
const faviconWatchList = new WeakSet();

function watchFavicon(img) {
  if (!(img instanceof HTMLImageElement) || faviconWatchList.has(img)) return;
  faviconWatchList.add(img);
  const check = () => {
    if (!img.isConnected || !img.complete) return;
    if (img.naturalWidth <= 1 || img.naturalHeight <= 1) {
      fallbackFavicon(img, img.dataset.faviconLetter || '?');
    }
  };
  img.addEventListener('load', check);
  img.addEventListener('error', check);
  // Cached 1×1 fallback responses may finish before the inline handler runs.
  setTimeout(check, 0);
}

function scanFaviconImages(root = document) {
  if (!root || !root.querySelectorAll) return;
  const images = root.matches?.('img.favicon[data-favicon-letter]')
    ? [root]
    : root.querySelectorAll('img.favicon[data-favicon-letter]');
  images.forEach(watchFavicon);
}

if (typeof MutationObserver !== 'undefined' && document.documentElement) {
  const faviconObserver = new MutationObserver(records => {
    records.forEach(record => record.addedNodes.forEach(node => {
      if (node.nodeType === Node.ELEMENT_NODE) scanFaviconImages(node);
    }));
  });
  faviconObserver.observe(document.documentElement, { childList: true, subtree: true });
  scanFaviconImages();
}
...
function faviconHtml(siteUrl, name, size = 17) {
  ...
  const src = `/favicons?domain_url=${encodeURIComponent(faviconTargetUrl(siteUrl, d))}&sz=${Math.max(32, safeSize * 4)}&v=3`;
  const fallback = escapeJsString(letter);
  return `<img class="favicon" style="--icon-size:${safeSize}px" src="${escapeHtml(src)}" data-favicon-letter="${escapeHtml(letter)}" loading="eager" referrerpolicy="no-referrer"
    onload="if (this.naturalWidth <= 1 || this.naturalHeight <= 1) fallbackFavicon(this, '${fallback}')"
    onerror="fallbackFavicon(this, '${fallback}')" />`;
}
...
/* ---------- API ---------- */
function aiScope() {
  if (state.authBypass) return 'guest';
...
async function loadTweetDrafts({ render = true } = {}) {
  if (!state.me) {
    state.tweetDrafts = [];
    state.tweetDraftEntries = [];
    if (render) renderSidebar();
    return [];
  }
  try {
    const data = await api('/api/me/tweet-drafts?limit=200');
    state.tweetDrafts = Array.isArray(data.drafts) ? data.drafts : [];
    state.tweetDraftEntries = state.tweetDrafts
      .map(item => item && item.entry)
  ...
  if (render) {
    renderSidebar();
    if (state.view === 'tweet-drafts') renderList();
  }
  return state.tweetDrafts;
}

async function loadMe() {
  const data = await api('/api/me');
  state.authBypass = Boolean(data.authBypass);
...
function renderSidebar() {
  const groups = { pinned: [], news: [], article: [], podcast: [] };
  for (const s of state.sources) {
    if (!s.enabled) continue;
    if (s.pinned) groups.pinned.push(s);
    else groups[s.category]?.push(s);
  }
  const groupLabels = { pinned: '置顶来源', ...CATEGORY_LABELS };
  ...
  for (const [cat, list] of Object.entries(groups)) {
    ...
    label.textContent = groupLabels[cat];
    if (cat !== 'pinned') {
      label.style.cursor = 'pointer';
      label.title = `查看全部${groupLabels[cat]}`;
    ...
    for (const s of list) {
      ...
      pin.type = 'button';
      pin.className = `feed-pin-btn${s.pinned ? ' active' : ''}`;
      pin.title = s.pinned ? '取消置顶来源' : '置顶来源';
      pin.setAttribute('aria-label', pin.title);
      pin.setAttribute('aria-pressed', s.pinned ? 'true' : 'false');
      pin.innerHTML = iconMarkup('pin', { className: 'app-icon' });
      pin.onclick = async event => {
        event.stopPropagation();
        pin.disabled = true;
        try {
          const result = await api(`/api/sources/${encodeURIComponent(s.id)}/pin`, { method: 'POST' });
          s.pinned = Boolean(result.pinned);
          toast(s.pinned ? `${s.name} 已置顶` : `${s.name} 已取消置顶`);
          renderSidebar();
        } catch (error) {
          toast(`设置置顶失败: ${error.message}`, 5000);
          pin.disabled = false;
        }
      };
      row.append(btn, pin);
  ...
  $('#count-tweet-drafts').textContent = state.tweetDrafts.length || '';
  $('#count-contributors').textContent = state.contributors.length || '';
  enforcePersonalUiVisibility();
...
function enforcePersonalUiVisibility() {
  const selectors = [
    '.view-btn[data-view="hot"]',
    '.view-btn[data-view="contributors"]',
    '#submit-link-open',
    '#article-link-submit',
    '#account-menu-profile',
    '#reader-rail-like',
    '#reader-rail-annotation',
    '#reader-like',
    '#context-tab-annotations',
    '[data-dashboard-tab="contributions"]',
    '[data-my-asset-tab="annotations"]',
    '[data-my-asset-tab="comments"]',
    '[data-my-asset-tab="chat"]',
    '[data-contributor-tab="annotations"]',
    '[data-contributor-tab="comments"]',
    '[data-contributor-tab="chat"]',
    '[data-contributor-tab="likes"]',
  ];
  ...
  $$('.asset-filter-chip').forEach(chip => {
    const type = chip.dataset.assetStripFilter;
    if (type && ASSET_FILTERS[type]?.visible === false) {
      chip.hidden = true;
...
function entryHasAssetType(entry, type) {
  if (type === 'rewrite') {
    return assetCountForType(entry, 'rewrite') > 0 || assetCountForType(entry, 'translation') > 0;
  }
...
function contributorSearchText(contributor) {
  return [
    ...
    `${contributor.translationCount || 0} 改写`,
...
function visibleEntries() {
  let list = state.view === 'tweet-drafts'
    ? [...state.entries, ...(state.tweetDraftEntries || [])]
    : state.entries;
  if (state.view === 'tweet-drafts') {
    list = [...new Map(list.filter(entry => entry && entry.id).map(entry => [entry.id, entry])).values()];
  }
  if (state.view === 'hot') {
    list = list
      .slice()
  ...
  if (state.view === 'tweet-drafts') {
    const drafts = new Map((state.tweetDrafts || []).map(item => [item.entryId, item]));
    list = list
      .filter(entry => drafts.has(entry.id))
      .filter(entry => {
        const draft = drafts.get(entry.id);
        return entryMatchesSearch(entry, { includeAssets: true })
          || normalizeSearchText(draft && `${draft.draft || ''} ${draft.userInput || ''}`).includes(normalizeSearchText(state.q));
      })
      .slice()
      .sort((a, b) => (Number(drafts.get(b.id)?.updatedAt) || 0) - (Number(drafts.get(a.id)?.updatedAt) || 0));
  }
...
function entryAssetItems(entry) {
  const assets = entry && entry.assets ? entry.assets : {};
  const items = [];
  const translationCount = assetCountForType(entry, 'translation');
  const rewriteCount = assetCountForType(entry, 'rewrite');
  if (translationCount || rewriteCount) {
    items.push({
      // Prefer the rewrite focus when both legacy forms exist; a
      // translation-only entry still opens its compatible translation view.
      type: rewriteCount ? 'rewrite' : 'translation',
      label: '中文改写',
      count: translationCount + rewriteCount,
      title: '查看中文改写',
    });
  }
...
const ASSET_FILTERS = {
  // `translation` is retained as a legacy filter for old links, but the
  // personal reader presents it under the unified Chinese rewrite concept.
  translation: { label: '改写', count: entry => assetCountForType(entry, 'translation'), title: '查看历史中文改写资产', visible: false },
  rewrite: { label: '改写', count: entry => assetCountForType(entry, 'rewrite'), title: '查看中文改写资产（含历史双语改写）' },
  annotations: { label: '划线', count: entry => assetCountForType(entry, 'annotations'), title: '查看有划线点评的文章', visible: false },
  comments: { label: '点评', count: entry => assetCountForType(entry, 'comments'), title: '查看有人工点评的文章', visible: false },
  chat: { label: '对话', count: entry => assetCountForType(entry, 'chat'), title: '查看有文章对话的文章', visible: false },
};
...
function assetTypeCount(entries, type) {
  ...
  if (!def) return 0;
  if (type === 'rewrite') {
    return entries.reduce((sum, entry) => sum + assetCountForType(entry, 'rewrite') + assetCountForType(entry, 'translation'), 0);
  }
  return entries.reduce((sum, entry) => sum + def.count(entry), 0);
}

function assetTotalCount(entries) {
  return ['rewrite', 'annotations', 'comments', 'chat']
    .reduce((sum, type) => sum + assetTypeCount(entries, type), 0);
}
...
function assetLatestAtForType(entry, type = '') {
  ...
  if (type === 'rewrite') {
    const latestAt = (assetType) => {
      const itemAt = Number((assets.items && assets.items[assetType] && assets.items[assetType][0] && assets.items[assetType][0].at) || 0);
      const previewAt = Number((assets.previews && assets.previews[assetType] && assets.previews[assetType].at) || 0);
      return Math.max(itemAt, previewAt);
    };
    return Math.max(latestAt('rewrite'), latestAt('translation'));
  }
...
function assetHelpfulScoreForType(entry, type = '') {
  ...
  if (type === 'rewrite') return (Number(assets.rewriteHelpfulCount) || 0) + (Number(assets.translationHelpfulCount) || 0);
...
function assetHelpfulItemCount(entry, type = '') {
  ...
  if (type === 'rewrite') return helpfulAiAssetItemCount(assets, 'rewrite') + helpfulAiAssetItemCount(assets, 'translation');
...
function assetDashboardStats() {
  ...
  return {
    ...
    helpfulEntries,
    helpfulTotal,
    totalAssets: assetTotalCount(entries),
  };
}
...
function assetPreviewForEntry(entry) {
  ...
  if (
    ...
    && (entry?.assets?.topHelpfulRewrite || entry?.assets?.topHelpfulTranslation)
  ) {
    return entry.assets.topHelpfulRewrite || entry.assets.topHelpfulTranslation;
  }
  if (
    state.assetSort === 'helpful'
...
function assetPreviewForType(entry, type = '') {
  const assets = entry && entry.assets ? entry.assets : {};
  const previews = assets.previews || {};
  const preview = type === 'rewrite'
    ? (previews.rewrite || previews.translation || assets.preview)
    : type && previews[type] ? previews[type] : assets.preview;
...
function renderEntryPaneTabs() {
  ...
  if (!tabs) return;
  // The personal reader now uses the single top-level scope bar. Keep this
  // legacy node hidden so old bookmarks/DOM integrations do not reintroduce
  // the removed second row.
...
function renderListScopeBar() {
  ...
  if (!bar) return;
  const hidden = state.view === 'contributors' || state.view === 'tweet-drafts';
...
function selectListScope(scope = 'latest') {
  const next = ['latest', 'assets', 'hot', 'unread'].includes(scope) ? scope : 'latest';
...
function renderHomeAssetActivityList(el) {
  ...
  if (!items.length) {
    el.innerHTML = '<div class="list-empty">还没有已改写资产<br/>中文改写、点评或对话后会出现在这里</div>';
    return;
  }
...
function renderAssetActivityStrip() {
  ...
  if (state.view === 'assets') {
    ...
    const chips = [
      ...
      ...Object.entries(ASSET_FILTERS).filter(([, def]) => def.visible !== false).map(([type, def]) => {
...
function renderAuthState() {
  ...
  const localMode = Boolean(state.authBypass);
  ...
  if (loggedIn) {
    const displayName = localMode ? '本地模式' : (state.me.displayName || '读者');
    const accountLabel = localMode ? '免登录 · 本地管理员' : (isAdmin() ? '管理员' : '个人后台');
    ...
    `;
    $('#account-info').title = localMode ? '本地免登录模式' : '打开个人后台';
  ...
  $('#account-menu-password')?.classList.toggle('hidden', localMode);
  $('#account-menu-logout')?.classList.toggle('hidden', localMode);
...
function requireAuth(mode = 'login') {
  if (state.me || state.authBypass) return true;
  openAuth(mode);
  return false;
}
    ...
    card.innerHTML = `
      ...
      <div class="contributor-main">
        ...
        <div class="contributor-stats">
          ...
          <span><strong>${Number(contributor.translationCount || 0)}</strong> 改写</span>
...
function renderList() {
  ...
  $('#app').classList.toggle('view-tweet-drafts', state.view === 'tweet-drafts');
  $('#app').classList.remove('home-assets');
  renderListScopeBar();
  renderEntryPaneTabs();
  $('#mark-read-btn').classList.toggle('hidden', state.view === 'contributors');
  if (state.view === 'contributors') {
  ...
  if (!list.length) {
    ...
    const text = state.view === 'assets' && state.q
      ...
      : state.view === 'assets'
      ? '还没有沉淀资产<br/>先生成中文改写、点评或对话一篇文章'
      : state.view === 'history'
      ? '还没有浏览记录<br/>打开几篇文章后会出现在这里'
      : state.view === 'tweet-drafts'
      ? '还没有推文草稿<br/>打开一篇文章后生成第一版推文'
      : state.view === 'hot'
  ...
  const frag = document.createDocumentFragment();
  const draftMap = new Map((state.tweetDrafts || []).map(item => [item.entryId, item]));
  for (const e of visibleList) {
    const src = sourceById(e.sourceId);
    const assetsHtml = assetBadgesHtml(e, { interactive: true });
    const tweetDraft = state.view === 'tweet-drafts' ? draftMap.get(e.id) : null;
    const tweetDraftLabel = tweetDraft
      ? `推文草稿 · ${formatAssetTime(tweetDraft.updatedAt)}`
      : '';
    const entryActivity = tweetDraftLabel || assetActivityLabel(e) || entryHistoryLabel(e) || hotEntryLabel(e);
    ...
    const metaRow = [
      ...
      tweetDraft ? `<span class="tweet-draft-badge">推文草稿</span>` : '',
    ].filter(Boolean).join('');
...
function updateListTitle() {
  ...
  else if (state.view === 'tweet-drafts') title = '推文草稿';
...
function updateSearchPlaceholder() {
  ...
  search.placeholder = state.view === 'contributors'
    ...
    : state.view === 'assets'
      ? '搜索资产…'
      : state.view === 'tweet-drafts' ? '搜索推文草稿…' : '搜索文章…';
...
function renderTranslation(translation, { loading = false } = {}) {
  ...
  mode.title = state.translationCompare ? '切回纯中文改写' : '显示双语对照';
  ...
  if (loading) {
    ...
    if (emptyText) emptyText.textContent = `正在检查这篇文章的${TRANSLATION_REWRITE_LABEL}缓存…`;
  ...
  if (!hasContent) {
    empty.classList.remove('hidden');
    if (emptyText) emptyText.textContent = `这篇文章还没有${TRANSLATION_REWRITE_LABEL}。`;
    action.disabled = false;
    action.textContent = `生成${TRANSLATION_REWRITE_LABEL}`;
  ...
  action.disabled = false;
  action.textContent = translation.stale ? `更新${TRANSLATION_REWRITE_LABEL}` : `重新生成${TRANSLATION_REWRITE_LABEL}`;
...
function rewriteStatusText(status = state.rewriteStatus, rewrite = state.rewrite) {
  if (status === 'loading') return '正在读取已保存的中文改写…';
  if (status === 'generating') return '正在生成中文改写…';
  if (status === 'saved') return '中文改写已保存';
  if (status === 'cached') return '已显示缓存中文改写';
  if (status === 'error') return '中文改写生成失败，可重试';
  if (rewrite && rewrite.stale) return '原文或链接已更新，可重新生成';
  return '';
}

function renderRewriteStatus(status = state.rewriteStatus, rewrite = state.rewrite) {
  const el = $('#rewrite-status');
  if (!el) return;
  const text = rewriteStatusText(status, rewrite);
  el.textContent = text;
  el.classList.toggle('hidden', !text);
  el.classList.toggle('is-busy', status === 'loading' || status === 'generating');
  el.classList.toggle('is-error', status === 'error');
  el.classList.toggle('is-success', status === 'saved' || status === 'cached');
}
...
function renderRewrite(rewrite) {
  ...
  renderRewriteStatus(state.rewriteStatus, state.rewrite);
...
function copyRewriteText() {
  const rendered = elementTextForCopy($('#rewrite-content'));
  const fallback = plainTextFromHtml(state.rewrite && state.rewrite.body);
  copyText(rendered || fallback, rewriteUiCopy().copied);
}

function tweetSystemPromptValue() {
...
function tweetDraftReferenceText(entry = state.activeEntry, draft = null) {
  if (!entry) return '请先选择一篇文章。';
  const source = sourceById(entry.sourceId);
  const parts = [entry.titleZh || entry.title || '无标题', source?.name || '文章'];
  if (draft?.stale) parts.push('文章内容有更新，建议重新生成');
  else parts.push('原文在左侧阅读');
  return parts.join(' · ');
}

function invalidateTweetDraftRequests({ abortGeneration = true, abortLoad = true } = {}) {
  state.tweetDraftRequestSeq += 1;
  if (abortGeneration && state.tweetDraftAbortController) {
    state.tweetDraftAbortController.abort();
    state.tweetDraftAbortController = null;
  }
  if (abortLoad && state.tweetDraftLoadController) {
    state.tweetDraftLoadController.abort();
    state.tweetDraftLoadController = null;
  }
...
function renderTweetDraft() {
  ...
  const stop = $('#tweet-draft-stop');
  const clear = $('#tweet-draft-clear');
  const status = $('#tweet-draft-status');
  const reference = $('#tweet-draft-reference');
  ...
  if (copy) copy.disabled = !state.tweetDraft || state.tweetDraftBusy || state.tweetDraftLoading;
  if (stop) {
...
async function loadTweetDraft(entry = state.activeEntry) {
  ...
  if (state.tweetDraftLoadController) state.tweetDraftLoadController.abort();
  const requestSeq = ++state.tweetDraftRequestSeq;
  const controller = new AbortController();
  state.tweetDraftLoadController = controller;
  state.tweetDraftRecord = null;
  state.tweetDraft = '';
  ...
  renderTweetDraft();
  try {
    const data = await api(`/api/entry/${encodeURIComponent(entry.id)}/tweet-draft`, { signal: controller.signal });
    if (requestSeq !== state.tweetDraftRequestSeq || state.activeEntry?.id !== entry.id) return null;
    ...
    if (error?.name === 'AbortError' || requestSeq !== state.tweetDraftRequestSeq) return null;
    state.tweetDraftStatus = `加载失败：${error.message}`;
    return null;
  } finally {
    if (requestSeq === state.tweetDraftRequestSeq) {
      state.tweetDraftLoading = false;
      state.tweetDraftLoadController = null;
      renderTweetDraft();
    }
...
async function saveCurrentTweetDraft({ immediate = false } = {}) {
  ...
  try {
    ...
    if (requestSeq !== state.tweetDraftRequestSeq || state.activeEntry?.id !== entry.id) return null;
    state.tweetDraftRecord = data.draft || null;
    state.tweetDraftStatus = '草稿已保存。';
    await loadTweetDrafts({ render: false });
    renderSidebar();
    if (state.view === 'tweet-drafts') renderList();
    return state.tweetDraftRecord;
  } catch (error) {
    if (requestSeq === state.tweetDraftRequestSeq) state.tweetDraftStatus = `保存失败：${error.message}`;
    return null;
  } finally {
    if (requestSeq === state.tweetDraftRequestSeq) {
...
function queueTweetDraftSave() {
  if (!state.activeEntry || state.tweetDraftBusy || state.tweetDraftLoading) return;
  if (state.tweetDraftSaveTimer) clearTimeout(state.tweetDraftSaveTimer);
  state.tweetDraftSaveTimer = setTimeout(() => {
    state.tweetDraftSaveTimer = null;
    saveCurrentTweetDraft();
  }, 550);
}

async function clearTweetDraft() {
  const entry = state.activeEntry;
  if (!entry || state.tweetDraftBusy || state.tweetDraftLoading) return;
  if (state.tweetDraftSaveTimer) {
    clearTimeout(state.tweetDraftSaveTimer);
    state.tweetDraftSaveTimer = null;
  }
  try {
    await api(`/api/entry/${encodeURIComponent(entry.id)}/tweet-draft`, { method: 'DELETE' });
    if (state.activeEntry?.id !== entry.id) return;
    state.tweetDraftRecord = null;
    state.tweetDraft = '';
    state.tweetUserInput = '';
    ...
    state.tweetDraftStatus = `清空失败：${error.message}`;
    renderTweetDraft();
  }
}

function closeTweetDraft() {
  if (state.tweetDraftBusy) return;
  setAgentCollapsed(true);
}

function saveTweetSystemPrompt(value) {
  const next = String(value || '').slice(0, 8000).trim() || DEFAULT_TWEET_SYSTEM_PROMPT;
  state.tweetSystemPrompt = next;
  storage.setItem(TWEET_SYSTEM_PROMPT_STORAGE_KEY, next);
  storage.setItem(TWEET_SYSTEM_PROMPT_VERSION_KEY, TWEET_SYSTEM_PROMPT_VERSION);
  return next;
}
...
async function generateTweetDraft() {
  ...
  try {
    ...
    if (requestSeq !== state.tweetDraftRequestSeq || state.activeEntry?.id !== entry.id) return;
    ...
    if (requestSeq !== state.tweetDraftRequestSeq) return;
    if (error?.name === 'AbortError') {
      state.tweetDraftStatus = '已停止本次生成。';
      state.tweetProgress = 18;
      return;
    }
    state.tweetDraftStatus = `生成失败：${error.message}`;
    if (/API Key|未配置|Authentication|authentication|invalid_request_error|401/i.test(error.message)) {
      openAiConfigModal('rewrite', 'rewrite');
    }
    toast(`推文生成失败：${error.message}`, 5000);
  } finally {
    if (requestSeq === state.tweetDraftRequestSeq) {
...
async function loadRewrite(entry) {
  ...
  try {
    ...
    state.rewriteStatus = data.rewrite && data.rewrite.body ? 'cached' : 'idle';
    renderRewriteStatus(state.rewriteStatus, state.rewrite);
...
async function generateTranslation({ force = false } = {}) {
  ...
  btn.disabled = true;
  btn.textContent = `${TRANSLATION_REWRITE_LABEL}中…`;
  try {
    ...
    setReaderTab('translation');
    toast(data.originalFetched ? `已获取原文并保存${TRANSLATION_REWRITE_LABEL}` : data.cached ? `已显示缓存${TRANSLATION_REWRITE_LABEL}` : `${TRANSLATION_REWRITE_LABEL}已保存`);
  } catch (err) {
    ...
    toast(`${TRANSLATION_REWRITE_LABEL}失败: ` + err.message, 5000);
  } finally {
    state.translationGenerating = false;
    btn.disabled = false;
    if (!state.translation) btn.textContent = `生成${TRANSLATION_REWRITE_LABEL}`;
    else btn.textContent = state.translation.stale ? `更新${TRANSLATION_REWRITE_LABEL}` : `重新生成${TRANSLATION_REWRITE_LABEL}`;
  }
}
...
async function generateRewrite({ force = false } = {}) {
  ...
  state.rewriteStatus = 'generating';
  renderRewriteStatus(state.rewriteStatus, state.rewrite);
  btn.disabled = true;
  ...
  try {
    ...
    if (data.entry) applyServerEntryUpdate(data.entry);
    renderRewrite(data.rewrite);
    state.rewriteStatus = data.cached ? 'cached' : 'saved';
    renderRewriteStatus(state.rewriteStatus, state.rewrite);
    if (data.rewrite && data.rewrite.body) {
      updateEntryAssets(entry.id, entryAssetHelpfulPatch('rewrite', data.rewrite), { rerenderList: false });
      renderList();
...
function myAssetUrl(type, item) {
  ...
  const actualType = item.legacyAssetType || type;
  if (actualType === 'translation' || actualType === 'rewrite') return readerAssetUrl(actualType, entry, item.id);
...
function renderMyAssetTabs() {
  ...
  const rewriteCount = $('#my-rewrite-count');
  if (rewriteCount) rewriteCount.textContent = counts.rewrite;
...
async function openMyCommentsModal({ push = true, tab = state.dashboardTab } = {}) {
  ...
  document.title = `个人后台 · ${PRODUCT_NAME}`;
  ...
  try {
    ...
    state.myRewrites = [
      ...(rewriteData.rewrites || []),
      ...state.myTranslations.map(item => ({
        ...item,
        type: 'rewrite',
        legacyAssetType: 'translation',
      })),
    ];
...
function userAssetDisplay(type, item) {
  ...
  if (type === 'translation') return { label: '中文改写', body: item.contentSnippet || item.summaryZh || '' };
  if (type === 'rewrite' && item && item.legacyAssetType === 'translation') {
    return { label: '中文改写', body: item.contentSnippet || item.summaryZh || '' };
  }
...
async function copyAssetContent(type, item) {
  const assetType = normalizeUserAssetTab(item?.legacyAssetType || type);
  if (!item) {
    toast('找不到这条资产');
    return;
  }
...
async function openMyAsset(itemId) {
  ...
  const type = normalizeUserAssetTab(item.legacyAssetType || state.myAssetTab);
...
function contributorPageTitle() {
  ...
  if (!profile) return `贡献主页 · ${PRODUCT_NAME}`;
  ...
  return `${sortPrefix}${profile.displayName} 的${label} · ${PRODUCT_NAME}`;
}
...
function renderContributorAssets() {
  ...
  $('#contributor-subtitle').textContent = profile
    ? `公开沉淀的中文改写、划线点评、人工点评、文章对话和点赞文章。${helpfulCount ? `获得 ${helpfulCount} 次有用反馈，覆盖 ${helpfulAssets} 条资产。` : ''}`
    : '正在读取公开资产…';
...
async function openEntry(e, { tab = null, focus = null, aiAssetId = '', commentId = '', annotationId = '', chatMessageId = '', updateUrl = true, replaceUrl = false } = {}) {
  ...
  if (focus === 'chat' || chatMessageId) {
    setContextPanel('agent', { expand: true });
  } else if (state.contextPanel !== 'agent' && state.contextPanel !== 'annotations') {
    setContextPanel('tweet', { expand: false });
  }
  state.activeEntry = e;
...
async function openEntryFromUrl({ reuseLoadedCollections = false } = {}) {
  ...
  if (!route.entryId) {
    if (route.view === 'contributors') {
      ...
      state.contributorSort = 'latest';
      state.q = route.q;
    } else if (route.view === 'tweet-drafts') {
      state.view = 'tweet-drafts';
      state.filterSource = null;
      state.filterCategory = null;
      state.assetFilter = null;
    ...
    if (route.view === 'assets' || route.view === 'contributors' || route.view === 'tweet-drafts') document.title = listRouteTitle();
    return false;
  }
  try {
...
function opsStatusText(value) {
  ...
  if (text === 'manual-only') return '中文改写需手动生成';
  return text;
}
...
function autoRewriteStatusParts() {
  ...
  const titleTranslation = last.kind === 'title-translation';
  const running = Boolean(auto.running || last.running);
  const failed = [
    ...(last.error ? [{ title: titleTranslation ? '标题翻译任务' : '手动改写任务', error: last.error }] : []),
    ...(Array.isArray(last.failed) ? last.failed : []),
  ];
  if (running) {
    return {
      label: titleTranslation ? '标题翻译中' : '手动改写中',
      value: '后台运行',
  ...
  if (!last.startedAt) {
    return { label: '手动改写', value: '待命', meta: '刷新只补标题，改写需手动生成', failed };
  }
  ...
  return {
    label: titleTranslation ? '标题翻译完成' : '手动改写完成',
    value,
    ...
    <div class="manage-status-grid">
      ...
      <div class="manage-status-item ${state.autoRewrite?.running ? 'active' : failures.length ? 'error' : ''}">
        ...
        <div class="manage-status-action-row">
          ...
          <button id="${actionId}" class="manage-status-action" type="button" ${state.autoRewrite?.running ? 'disabled' : ''}>手动批量生成</button>
        </div>
...
async function runAutoRewriteFromManage() {
  ...
  try {
    ...
    toast('启动手动批量改写失败: ' + error.message, 5000);
...
async function openAdminPage({ push = true } = {}) {
  ...
  state.activeEntry = null;
  document.title = `系统后台 · ${PRODUCT_NAME}`;
  renderAdminPage();
  if (push) {
...
function renderTweetSystemPromptSettings() {
  const prompt = $('#tweet-system-prompt');
  if (prompt) {
...
async function deleteAiProfile() {
  ...
  const ok = await showConfirmDialog({
    title: '删除 AI 配置',
    message: `确定删除「${profile.name}」吗？使用这个配置的中文改写和对话会切回默认配置。`,
    confirmText: '删除',
    danger: true,
  });
  if (!ok) return;
...
function setContextPanel(panel = 'tweet', { persist = true, expand = false } = {}) {
  const next = ['tweet', 'agent', 'annotations'].includes(panel) ? panel : 'tweet';
  state.contextPanel = next;
  if (persist) storage.setItem('qm_context_panel', next);
  const isTweet = next === 'tweet';
  const isAgent = next === 'agent';
  const isAnnotations = next === 'annotations';
  const tab = (selector) => $(selector);
  const tweetTab = tab('#context-tab-tweet');
  const agentTab = tab('#context-tab-agent');
  const annotationTab = tab('#context-tab-annotations');
  const tweetPanel = tab('#tweet-side-panel');
  const agentPanel = tab('#agent-side-panel');
  const annotationPanel = tab('#annotation-side-panel');
  [tweetTab, agentTab, annotationTab].forEach((node) => {
    if (!node) return;
    const visible = node === tweetTab ? isTweet : node === agentTab ? isAgent : isAnnotations;
    node.classList.toggle('hidden', !visible);
    node.hidden = !visible;
    node.classList.toggle('active', visible);
    node.setAttribute('aria-pressed', visible ? 'true' : 'false');
  });
  [[tweetPanel, isTweet], [agentPanel, isAgent], [annotationPanel, isAnnotations]].forEach(([node, visible]) => {
    if (!node) return;
    node.classList.toggle('hidden', !visible);
    node.hidden = !visible;
  });
  const tabs = $('#agent-pane .context-tabs');
  tabs?.classList.toggle('agent-only-tabs', isTweet);
  placeContextCloseButton(next);
  $('#app')?.classList.toggle('context-tweet-active', isTweet);
  $('#app')?.classList.toggle('context-agent-active', isAgent);
  $('#app')?.classList.toggle('context-annotations-active', isAnnotations);
  syncContextOpenRail();
...
function placeContextCloseButton(panel = state.contextPanel) {
  const closer = $('#context-close');
  const contextCloseSlot = $('#context-close-slot');
  const tweetCloseSlot = $('#tweet-side-close-slot');
  const target = panel === 'tweet' ? tweetCloseSlot : contextCloseSlot;
  if (closer && target && closer.parentElement !== target) target.appendChild(closer);
}

function syncContextOpenRail() {
  const showFloatingOpener = Boolean(state.agentCollapsed && state.contextPanel !== 'tweet');
  $('#context-open-rail')?.classList.toggle('hidden', !showFloatingOpener);
  $('#agent-open')?.classList.toggle('hidden', !showFloatingOpener);
}

function setAgentCollapsed(collapsed, { persist: shouldPersist = true } = {}) {
...
function normalizeReaderWorkbenchLayout() {
  ...
  // Left navigation and the right writing desk are independent user controls.
  // Responsive CSS hides or stacks panes at narrow widths; it must not rewrite
  // either persisted collapse preference as a side effect of opening the other.
}
...
async function deleteCurrentEntry() {
  ...
  const ok = await showConfirmDialog({
    title: '删除页面',
    message: `确认删除《${title}》？这会从前台列表、文章页、公开资产目录中隐藏该页面；已沉淀的中文改写、点评、划线和对话数据不会被清空。`,
    confirmText: '删除页面',
    danger: true,
  });
  if (!ok) return;
...
const readerTweetOpen = $('#reader-tweet-open');
if (readerTweetOpen) {
  readerTweetOpen.onclick = () => {
    setContextPanel('tweet', { expand: true });
    window.requestAnimationFrame(() => $('#tweet-user-input')?.focus());
  };
}
...
$('#tweet-output').oninput = (event) => {
  state.tweetDraft = String(event.target.value || '').slice(0, 20000);
  queueTweetDraftSave();
};
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Escape') {
    ...
    $('#submit-link-modal').classList.add('hidden');
    $('#agent-prompt-modal').classList.add('hidden');
    if (!state.tweetDraftBusy) closeTweetDraft();
    hideArticleLinkMenu();
